...
AF_f <- file.path(ResultDir,"Exac_AF.RDS")
...
Exac_AF_evid <- mutate(Exac_bed_full,evid=paste0(chrom,":",start,"-",end,"_",ref,"_",alt)) %>% 
  select(-chrom,-start,-end,-ref,-alt,evid,ExAC_AC=ac_adj,ExAC_AF=af_adj)
saveRDS(Exac_AF_evid,AF_f)
...
region_df <-  semi_join(region_df,
                        rename(
                          FindMax(select(region_df,chrom=chr,start,end,name=gene))
                          ,gene=name)) %>%
